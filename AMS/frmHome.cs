...
namespace AMS
{
    public partial class frmHome : Form
    {
        ...
        {
            var idStr = dgvProvincial.CurrentRow.Cells["ProvincialId"].Value.ToString();
            if(string.IsNullOrWhiteSpace(idStr))
            {
                MessageBox.Show("Chọn Tỉnh/Thành phố cần sửa.", "Thông báo");
                return;
            }

            if (MessageBox.Show("Bạn có chắc muốn cập nhật Tỉnh/Thành phố.?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                int provinceId = int.Parse(idStr);
                string queryStr = string.Format("Update tbProvince set Name = N'{0}' Where id = {1}", txtProvincial.Text, provinceId);
        ...
        {
            var idStr = dgvDistrict.CurrentRow.Cells["DistrictId"].Value.ToString();
            if (string.IsNullOrWhiteSpace(idStr))
            {
                MessageBox.Show("Chọn Quận/Huyện cần sửa.", "Thông báo");
                return;
            }

            if (MessageBox.Show("Bạn có chắc muốn cập nhật Quận/Huyện.?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                int districtId = int.Parse(idStr);
                string queryStr = string.Format("Update tbDistrict set Name = N'{0}' Where id = {1}", txtDistrict.Text, districtId);
                da.execSqlQuery(queryStr);
                BindDataDistrictList(int.Parse(dgvProvincial.CurrentRow.Cells["ProvincialId"].Value.ToString()));
            }
        }
        ...
        {
            var idStr = dgvWard.CurrentRow.Cells["WardId"].Value.ToString();
            if (string.IsNullOrWhiteSpace(idStr))
            {
                MessageBox.Show("Chọn Phường/Xã cần sửa.", "Thông báo");
                return;
            }

            if (MessageBox.Show("Bạn có chắc muốn cập nhật Phường/Xã.?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                int wardId = int.Parse(idStr);
                string queryStr = string.Format("Update tbWard set Name = N'{0}' Where id = {1}", txtWard.Text, wardId);
                da.execSqlQuery(queryStr);
                BindDataWardList(int.Parse(dgvDistrict.CurrentRow.Cells["DistrictId"].Value.ToString()));
            }
        }
        ...
        {
            var idStr = dgvStreet.CurrentRow.Cells["StreetIdd"].Value.ToString();
            if (string.IsNullOrWhiteSpace(idStr))
            {
                MessageBox.Show("Chọn Đường cần sửa.", "Thông báo");
                return;
            }

            if (MessageBox.Show("Bạn có chắc muốn cập nhật Đường.?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                int streetId = int.Parse(idStr);
                string queryStr = string.Format("Update tbStreet set Name = N'{0}' Where id = {1}", txtStreet.Text, streetId);
                da.execSqlQuery(queryStr);
                BindDataStreetList(int.Parse(dgvWard.CurrentRow.Cells["WardId"].Value.ToString()));
            }
        }
